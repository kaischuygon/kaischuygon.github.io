...
function updateTicTac() {
	for(i = 0; i < 9; i++) {
		var boxID = indexToGrid(i)
		switch(board_state[i]) {
			case -1:
				document.getElementById(boxID).innerHTML = boxID
				break;
			case 0:
...
function getIndex(input) {
	if(input[0] == "A") {
		...
		return parseInt(input[1]) + 2
	} else {
		return parseInt(input[1]) + 5
	}
}
...
function updateArray(input) {
	...
	if(board_state[index] == -1) {
		...
		alert("Cell already filled, try again.")
		changeTurn()
		return
	}
}
...
function play() {
	if(started == true) {
		var move = document.getElementById("move_text_id")
		var moveInput = /[A-C][1-3]/g;
		if(move.value.match(moveInput) && move.value.length == 2) {
			updateArray(move.value)
			changeTurn()
			displayTurn()
			move.value = ""
			checkWinner()
		} else {
			alert("Enter a valid move. For example: A1")
			move.value = ""
		}
	} else {
