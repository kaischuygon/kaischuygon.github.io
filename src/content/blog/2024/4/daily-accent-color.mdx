...
### Defining the colors
- I created a list of colors that I wanted to use as accent colors. I used the Tailwind CSS colors as a base.
...
- `define:vars` is Astro-specific, but you can use any method to set the default color.
- The color is set as a CSS variable `--ACCENT`.
```html
<script define:vars={{ defaultColor }}>
	...
	document.addEventListener('astro:page-load', () => {
		if (!localStorage.getItem('accent')) {
            ...
            document.getElementById('colorSwitch').value = defaultColor;
		} else {
	...
	addEventListener('astro:page-load', () => document.documentElement.classList.add('loaded'));
</script>

<style define:vars={{ defaultColor }} is:global>
	:root {
...
- Again, `is:inline` and `define:vars` are Astro-specific, but you can use any method to set the color.
...
<script is:inline define:vars={{ colorArray, defaultColor }}>
    document.addEventListener('astro:after-swap', () => {
        function changeAccent() {
            // Split the color data into an array
            const newAccent = document.getElementById('colorSwitch').value
            if (newAccent === 'daily') {
                document.documentElement.style.setProperty('--ACCENT', defaultColor)
                localStorage.setItem('accent', 'daily')
                return
        ...
        document.getElementById('colorSwitch').addEventListener('change', changeAccent)
    });
</script>
```

That is it! Now you have a daily accent color picker that works with Astro and Tailwind CSS. You can see the code for the full component `ColorSwitch.astro` [here](https://github.com/kaischuygon/kaischuygon.github.io/blob/8523b4038d302ada8421aa01d84b91d01975764b/src/components/ColorSwitch.astro).
